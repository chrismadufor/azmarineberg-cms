...
import { createRequest } from "@/services/dashboardService";
...
export default function RequestForm({
  ...
  const { services } = useSelector(
    (state) => state.dashboard || { services: [] }
  );
    ...
    if (Array.isArray(services) && services.length > 0) {
      const options = [
        ...
        ...services.map((service) => ({
          name:
            service.name || service.serviceName || service.title || "Unknown",
          value:
            service._id ||
            service.id ||
            service.name ||
                ...
                return (
                  <Form>
                    <div className="mb-5">
                      {preSelectedService ? (
                        <div>
                          <label className="text-gray-800 font-semibold text-sm mb-1 block">
                            Service
                          </label>
                          <input
                            type="text"
